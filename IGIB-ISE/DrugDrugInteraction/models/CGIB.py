...
class CGIB_ModelTrainer(embedder):
    def __init__(self, args, train_df, valid_df, test_df, repeat, fold):
        ...
        self.model = CGIB(device = self.device, num_step_message_passing = self.args.message_passing,EM=self.args.EM_NUM).to(self.device)
    ...
    def train(self):        
        ...
        for epoch in range(1, self.args.epochs + 1):
            ...
            for bc, samples in enumerate(self.train_loader):
                ...
                loss = loss_function_BCE(outputs, samples[2].reshape(-1, 1).to(self.device).float()).mean()
                #print(self.args.beta1)
                loss += self.args.beta * KL_Loss_1
                loss += self.args.beta * cont_loss_1
                loss += self.args.beta1 * KL_Loss_2
                loss += self.args.beta1 * cont_loss_2
                #print(loss)
                loss.backward()
                self.optimizer.step()
            ...
            if len(self.best_val_rocs) > int(self.args.es / self.args.eval_freq):
                if self.best_val_rocs[-1] == self.best_val_rocs[-int(self.args.es / self.args.eval_freq)]:
                    if self.best_val_accs[-1] == self.best_val_accs[-int(self.args.es / self.args.eval_freq)]:
                        ...
                        PATH='state_dict_model{},{},{}.pth'.format(self.args.beta,self.args.beta1,self.args.EM_NUM)
                        #先建立路径
...
class CGIB(nn.Module):
    ...
    def __init__(self,
        ...
        self.solute_predictor = nn.Linear(4 * self.node_hidden_dim, 4 * self.node_hidden_dim)
    ...
    def forward(self, data, bottleneck = False, test = False):
        ...
        if bottleneck:
            ...
            cont_loss_1 = self.mse_loss(noisy_solute_subgraphs, self.solvent_predictor(solvent_noisy_solute_subgraphs))
            cont_loss_2 = self.mse_loss(solvent_noisy_solute_subgraphs, self.solute_predictor(noisy_solute_subgraphs))
